...
#define ENDTABITEM_SPACED() ImGui::EndTabItem(); }
	...
	std::vector<std::string> fontNames = {"Default", "Fira Code", "DejaVu Sans Mono", "Ukij Qolyazma", "Ubuntu-Title"};
	...
	static void init(HDC hdc)
	{
		...
		std::string fontPath = get_font_path(L"DejaVu Sans Mono (TrueType)");
		const char *fontPathStr = validate_font_path(fontPath);
		io.Fonts->AddFontFromFileTTF(fontPathStr, 16.0f);
	...
	void draw_menu()
	{
		...
		ImGui::Begin("##", &displayed, ImGuiWindowFlags_NoCollapse);
	...
	void draw_menu_tab_iw1x()
	{
		BEGINTABITEM_SPACE("IW1X")
		...
		const char *preview = fontNames[selectedFontIndex].c_str();
		if (ImGui::BeginCombo("##combo_font", preview, flags))
		{
			for (int i = 0; i < fontNames.size(); i++)
			{
				printf("Debug: %s\n", fontNames[i].c_str());
				const bool isSelected = (selectedFontIndex == i);
				if (ImGui::Selectable(fontNames[i].c_str(), isSelected))
				{
					selectedFontIndex = i;
					//fonts.clear();
					//std::string fontPath = get_font_path(fontNames[selectedFontIndex]);
					//const char *fontPathStr = validate_font_path(fontPath);
					//fonts.push_back(ImGui::GetIO().Fonts->AddFontFromFileTTF(fontPathStr, 16.0f));
				}
				if (isSelected)
