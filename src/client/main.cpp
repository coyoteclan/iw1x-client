...
// Callback collects each face name exactly once
    ...
    LPARAM lParam)
{
    ...
    std::wstring face(lplf->lfFaceName);
    if (std::find(names.begin(), names.end(), face) == names.end())
...
std::vector<std::wstring> EnumerateSystemFonts()
{
    ...
    LOGFONTW lf = {};
    lf.lfCharSet = DEFAULT_CHARSET;           // all character sets
    lf.lfFaceName[0] = L'\0';                 // all face names
...
{
    /*std::vector<std::wstring> fontNames = EnumerateSystemFonts();
    for (const auto& fontName : fontNames)
    {
        printf("%ls\n", fontName.c_str());
        if (fontName == L"Fira Code")
        {
            //
        }
    }//*/
